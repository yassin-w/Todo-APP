...
class TableTodo extends SqfEntityTableBase {
  TableTodo() {
    ...
    fields = [
      ...
      SqfEntityFieldBase('long', DbType.real),
      SqfEntityFieldBase('lat', DbType.real),
      SqfEntityFieldBase('isComplete', DbType.bool, defaultValue: false),
    ];
    super.init();
  }
...
class Todo extends TableBase {
  ...
  Todo.withFields(this.name, this.description, this.date, this.dateCreated,
      this.image, this.userId, this.long, this.lat, this.isComplete) {
    _setDefaultValues();
  }
  Todo.withId(this.id, this.name, this.description, this.date, this.dateCreated,
      this.image, this.userId, this.long, this.lat, this.isComplete) {
    _setDefaultValues();
  }
  // fromMap v2.0
  Todo.fromMap(Map<String, dynamic> o, {bool setDefaultValues = true}) {
    ...
    if (o['long'] != null) {
      long = double.tryParse(o['long'].toString());
    }
    if (o['lat'] != null) {
      lat = double.tryParse(o['lat'].toString());
    }
    if (o['isComplete'] != null) {
      isComplete = o['isComplete'].toString() == '1' ||
          o['isComplete'].toString() == 'true';
    }
  }
  // FIELDS (Todo)
  int? id;
  ...
  Map<String, dynamic> toMap(
    ...
    if (isComplete != null) {
      map['isComplete'] = forQuery ? (isComplete! ? 1 : 0) : isComplete;
    } else if (isComplete != null || !forView) {
      map['isComplete'] = null;
    }
  ...
  Future<int?> upsert({bool ignoreBatch = true}) async {
    try {
      final result = await _mnTodo.rawInsert(
          'INSERT OR REPLACE INTO todos (id, name, description, date, dateCreated, image, userId, long, lat, isComplete)  VALUES (?,?,?,?,?,?,?,?,?,?)',
          [
            id,
            name,
            description,
...
class TodoFilterBuilder extends ConjunctionBase {
  ...
  TodoField get dateCreated {
    return _dateCreated = _setField(_dateCreated, 'dateCreated', DbType.text);
  }

  TodoField? _image;
  TodoField get image {
    return _image = _setField(_image, 'image', DbType.blob);
  }
  ...
  TodoField? _long;
  TodoField get long {
    return _long = _setField(_long, 'long', DbType.real);
  }

  TodoField? _lat;
  TodoField get lat {
    return _lat = _setField(_lat, 'lat', DbType.real);
  }

  TodoField? _isComplete;
  TodoField get isComplete {
    return _isComplete = _setField(_isComplete, 'isComplete', DbType.bool);
  }
...
class TodoFields {
  ...
  static TableField? _fLong;
  static TableField get long {
    return _fLong = _fLong ?? SqlSyntax.setField(_fLong, 'long', DbType.real);
  }

  static TableField? _fLat;
  static TableField get lat {
    return _fLat = _fLat ?? SqlSyntax.setField(_fLat, 'lat', DbType.real);
  }

  static TableField? _fIsComplete;
  static TableField get isComplete {
    return _fIsComplete = _fIsComplete ??
        SqlSyntax.setField(_fIsComplete, 'isComplete', DbType.bool);
  }
}
